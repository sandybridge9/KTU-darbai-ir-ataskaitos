...
public class BstSetKTU<E extends Comparable<E>> implements SortedSetADT<E>, Cloneable {
    ...
    public SortedSetADT<E> SubSet(E fromElement, boolean fromInclusive,
        ...
        for(E elem : array){
            if(fromInclusive == false && elem.compareTo(fromElement) > 0){
                if(toInclusive == false && elem.compareTo(toElement) < 0){
                    subSet.add(elem);
                }
                if(toInclusive == true && elem.compareTo(toElement) <= 0){
                    subSet.add(elem);
                }
            }
            else if (fromInclusive == true && elem.compareTo(fromElement) >= 0){
                if(toInclusive == false && elem.compareTo(toElement) < 0){
                    subSet.add(elem);
                }
                if(toInclusive == true && elem.compareTo(toElement) <= 0){
    ...
    public E lower(E e){
        Object[] objects = this.toArray();
        E maxElem = (E)objects[0];
        for(Object obj : objects){
            if(e.compareTo((E) obj) > 0){
                if(maxElem.compareTo((E) obj) < 0){
                    maxElem = (E) obj;
                }
            }
        }
