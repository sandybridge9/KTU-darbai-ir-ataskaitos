...
import lab3Laurinaitis.KaciuGamyba;
import lab3Laurinaitis.Kate;
import java.io.File;
...
import lab3Laurinaitis.GreitaveikosTyrimas;
import laborai.studijosktu.SortedSetADTx;

public class Lab3WindowFXMano extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private static SortedSetADTx<Kate> kaciuSet;
    private int sizeOfInitialSubSet, sizeOfGenSet, sizeOfLeftSubSet;
    private double coef;
    private final String[] errors;

    public Lab3WindowFXMano(Stage stage) {
        this.stage = stage;
        errors = new String[]{
    ...
    private void treeGeneration(String filePath) throws MyException {
        ...
        if (filePath == null) {
            kaciuArray = KaciuGamyba.generuotiIrIsmaisyti(sizeOfGenSet, sizeOfInitialSubSet, coef);
            paneParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            kaciuSet.load(filePath);
            kaciuArray = new Kate[kaciuSet.size()];
            int i = 0;
            for (Object o : kaciuSet.toArray()) {
                kaciuArray[i++] = (Kate) o;
            }
            // Skaitant iš failo išmaišoma standartiniu Collections.shuffle metodu.
            Collections.shuffle(Arrays.asList(kaciuArray), new Random());
        }
        ...
        KsFX.setFormatStartOfLine(true);
        KsFX.oun(taOutput, kaciuSet.toVisualizedString(tfDelimiter.getText()),
    ...
    private void treeAdd() throws MyException {
        KsFX.setFormatStartOfLine(true);
        Kate kate = KaciuGamyba.gautiIsBazes();
        kaciuSet.add(kate);
        paneParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));   
        KsFX.oun(taOutput, kate, MESSAGES.getString("msg7"));
        KsFX.oun(taOutput, kaciuSet.toVisualizedString(tfDelimiter.getText()));
        KsFX.setFormatStartOfLine(false);
    }
    ...
    private void treeRemove() {
        ...
        if (kaciuSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(kaciuSet.size());
            Kate kate = (Kate) kaciuSet.toArray()[nr];
            kaciuSet.remove(kate);
            KsFX.oun(taOutput, kate, MESSAGES.getString("msg6"));
            KsFX.oun(taOutput, kaciuSet.toVisualizedString(tfDelimiter.getText()));
        }
        KsFX.setFormatStartOfLine(false);
    }
    ...
    private void treeIteration() {
        ...
        if (kaciuSet.isEmpty()) {
            ...
            KsFX.oun(taOutput, kaciuSet, MESSAGES.getString("msg8"));
        }
        KsFX.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws MyException {
        switch (cmbTreeType.getSelectionModel().getSelectedIndex()) {
            case 0:
                kaciuSet = new BstSetKTUx(new Kate());
                break;
            case 1:
                kaciuSet = new AvlSetKTUx(new Kate());
                break;
            default:
                disableButtons(true);
    ...
    public static void createAndShowFXGUI(Stage stage) {
        ...
        Lab3WindowFXMano window = new Lab3WindowFXMano(stage);
