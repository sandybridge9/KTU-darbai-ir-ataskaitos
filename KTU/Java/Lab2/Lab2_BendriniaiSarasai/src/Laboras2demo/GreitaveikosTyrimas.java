...
// sekančio tyrimo metu gaunama normalizuoti įvertinimai su klase TimeKeeper
    ...
    //Mano greitaveikos metodai -------------------------------------------------------------------------------------------------

    static Kate[] kaciuBaze;
    static ListKTU<Kate> kSeries = new ListKTU<>();
    static Random rand = new Random();
    
    static void generuotiKates(int kiekis){
        String[] kVyrVardai = {"Sigis", "Morkus", "Bonifacas", "Giedrius", "Juodis", "Softis", "Azuolas"};
        String[] kMotVardai = {"Kore", "Sniege", "Morka", "Selme", "Giedre", "Amelija", "Siva"};
        String[] kVeisles = {"Meskenas", "Sfinksas", "Trumpaplauke", "Egzotas"};
        String[] kLyt = {"Vyr","Mot"};
        
        kaciuBaze = new Kate[kiekis];
        rand.setSeed(2017);
        for(int i=0;i<kiekis;i++){
            int pLyt = rand.nextInt(kLyt.length);
            int pVeisle = rand.nextInt(kVeisles.length);
            String vardas = "";
            if(pLyt == 0){
                int pVardas = rand.nextInt(kVyrVardai.length);
            ...
            double kAmzius = rand.nextInt(12-1)+1;
            double kKaina = rand.nextInt(36000);
            kaciuBaze[i]= new Kate(vardas, kVeisles[pVeisle], kLyt[pLyt], kAmzius, kKaina+1);
        }
        Collections.shuffle(Arrays.asList(kaciuBaze));
        kSeries.clear();
        for(Kate a: kaciuBaze) 
            kSeries.add(a);
    }
    
    void sisteminisTyrimasPowVsSqrt(){
    // Paruošiamoji tyrimo dalis
        System.out.println("prasideda pow vs sqrt tyrimas");
        Timekeeper tk = new Timekeeper(tiriamiKiekiai);
        for (int kiekis : tiriamiKiekiai){
    //  Greitaveikos bandymai ir laiko matavimai
            tk.start();
    ...
    void sisteminisTyrimasArrayListVsLinkedList(){
    // Paruošiamoji tyrimo dalis
        Timekeeper tk = new Timekeeper(tiriamiKiekiai);
        for (int kiekis : tiriamiKiekiai) {
    //  Greitaveikos bandymai ir laiko matavimai
        ...
        tk.start();
        for(int i = 1; i <= kiekis; i++){
            int x = arrayList.lastIndexOf(i*20);
        }
        tk.finish("ArrayList");
        for(int i = 1; i <= kiekis; i++){
            int x = arrayList.lastIndexOf(i*20);
   ...
   public static void main(String[] args){
        ...
        new GreitaveikosTyrimas().generuotiKates(15);
        for(Kate k : kaciuBaze){
            System.out.println(k);
        }
        //new GreitaveikosTyrimas().sisteminisTyrimasPowVsSqrt();
        new GreitaveikosTyrimas().sisteminisTyrimasArrayListVsLinkedList();
   }    
}
