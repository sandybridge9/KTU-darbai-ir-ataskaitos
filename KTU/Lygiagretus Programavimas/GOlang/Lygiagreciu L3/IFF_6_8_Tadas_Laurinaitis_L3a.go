...
func main() {
	...
	var transferChan = make(chan []DataStructure)
	...
	waitGroup.Add(5)
	for i := 0; i < 5; i++ {
		go writer(writerChan, controllerChan)
	}
	waitGroup.Add(5)
	for i := 0; i < 5; i++ {
		go reader(readerChan, controllerChan)
	}
	waitGroup.Wait()
}
...
func writeDataToFile(students [][]Student, dataStructures [][]DataStructure, finalData []DataStructure) {
	file, err := os.Create("IFF_6_8_Tadas_Laurinaitis_Lab3a_rez.txt")
	if err != nil {
		fmt.Println("Couldnt open file")
		log.Fatal(err)
	}
...
func transfer(students [][]Student, dataStructures [][]DataStructure, writerChan chan<- []Student, readerChan chan<- []DataStructure, transferChan <-chan []DataStructure) {
	defer waitGroup.Done()
	for i := 0; i < len(students); i++ {
		writerChan <- students[i]
	}
	for i := 0; i < len(dataStructures); i++ {
		readerChan <- dataStructures[i]
	}
	for {
...
func controller(controllerChan <-chan Message, transferChan chan<- []DataStructure) {
	defer waitGroup.Done()
	count := 100
	...
	count = 0
	for writersDone < 5 || readersDone < 5 {
